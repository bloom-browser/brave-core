...
void Recover::Start(
  ...
  if (phrase_split.size() == 16) {
    ...
    // TODO add custom error for this one
    callback(ledger::Result::CORRUPTED_DATA, 0);
    return;
  }
  ...
  std::vector<uint8_t> secret_key =
      braveledger_helper::Security::GetHKDF(new_seed);
  ...
  auto url_callback = std::bind(&Recover::OnRecover,
      this,
      _1,
      new_seed,
      callback);
...
void Recover::OnRecover(
  ...
  // TODO we don't get balance anymore
